...
def obtener_noticias():
    ...
    # Nos aseguramos de que la carpeta exista, si no, la creamos silenciosamente
    os.makedirs(carpeta_destino, exist_ok=True)
    
    # Armamos la ruta completa (ej. politica-argentina/2026-07-14.json)
    ruta_completa = os.path.join(carpeta_destino, nombre_archivo)

    # 2. Búsqueda y filtrado
    query = 'política argentina -"economía" -"dólar" -"internacional" -"mercados" -"inflación"'
    ...
    # 3. Guardamos el archivo en la ruta que armamos al principio
    with open(ruta_completa, 'w', encoding='utf-8') as f:
        json.dump(noticias_seleccionadas, f, ensure_ascii=False, indent=4)
        
    print(f"¡Listo! Archivo generado en: {ruta_completa}")
